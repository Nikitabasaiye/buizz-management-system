...
export function EventsPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--app-background)] text-[var(--app-foreground)]">
      <div className="mx-auto max-w-[1600px] px-3 pb-6 sm:px-5 lg:px-8">
        ...
        <section className="mt-6 grid gap-4 rounded-md border border-[var(--app-border)] bg-[var(--app-elevated)] p-3 shadow-[0_18px_58px_rgba(15,23,42,0.10)] backdrop-blur sm:mt-8 sm:p-5 lg:grid-cols-[280px_1fr]">
          ...
          <div className="min-w-0">
            <SectionTitle title={activeCategory === "All" ? "All Events" : `${activeCategory} Events`} />
              ...
              <div className="grid min-h-72 place-items-center rounded-md border border-dashed border-[var(--app-border)] bg-[var(--app-subtle)] p-8 text-center">
                <div>
                  ...
                  <button
                    ...
                    className="mt-5 inline-flex min-h-10 items-center justify-center rounded-md bg-[var(--color-brand-primary)] px-5 text-sm font-black text-white shadow-[0_16px_38px_rgba(236,27,114,0.34)] transition duration-200 hover:bg-[var(--color-brand-primary)]"
                  >
                    Clear Filters
                  </button>
                </div>
    ...
    <section className="relative mt-4 overflow-hidden rounded-md border border-white/10 bg-[#070b15] text-white shadow-[0_28px_90px_rgba(0,0,0,0.48)] sm:mt-6">
      ...
      <div className="relative grid min-h-[460px] content-between gap-8 p-4 sm:min-h-[500px] sm:p-6 lg:min-h-[520px] lg:p-8">
        <div className="max-w-2xl pt-10 sm:pt-16 lg:pt-20">
          <h1 className="text-4xl font-black leading-none text-white drop-shadow-[0_3px_0_rgba(236,27,114,0.62)] sm:text-6xl">
            Top Buizz Events
          </h1>
          <Link href="/events" className="mt-5 inline-flex min-h-11 w-fit items-center justify-center rounded-md bg-[var(--color-brand-primary)] px-5 text-sm font-black !text-white shadow-[0_16px_38px_rgba(236,27,114,0.34)] transition duration-200 hover:bg-[var(--color-brand-primary)]">
            Explore Events
          </Link>
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4">
          ...
          <div className="mt-3 flex items-center justify-between gap-2">
            ...
            <Link
              ...
              className="inline-flex min-h-8 translate-y-0 items-center rounded-md bg-[var(--color-brand-primary)] px-2.5 text-[11px] font-black !text-white opacity-100 shadow-[0_12px_28px_rgba(236,27,114,0.28)] transition duration-200 hover:bg-[var(--color-brand-primary)] sm:translate-y-2 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100"
            >
              Book
            </Link>
          </div>
  ...
  return (
    <button
      ...
      className={`group relative block min-h-32 overflow-hidden rounded-md bg-[#171927] text-left text-white shadow-sm ring-1 transition duration-200 hover:-translate-y-1 hover:shadow-xl sm:min-h-40 ${
        active ? "ring-[var(--color-brand-primary)]" : "ring-white/10"
      }`}
    >
  ...
  return (
    <aside className="rounded-md border border-[var(--app-border)] bg-[var(--app-subtle)] p-4 shadow-[0_18px_58px_rgba(0,0,0,0.12)] lg:sticky lg:top-24 lg:self-start">
      <div className="flex items-center justify-between gap-3">
        ...
        <button type="button" onClick={onReset} className="text-xs font-black text-[var(--color-brand-primary)] transition hover:text-[var(--color-brand-primary)]">
          Clear all
        </button>
      </div>
      ...
      <button
        type="button"
        className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-[var(--color-brand-primary)] px-5 text-sm font-black text-white shadow-[0_16px_38px_rgba(236,27,114,0.34)] transition duration-200 hover:bg-[var(--color-brand-primary)]"
      >
        Apply Filters
      </button>
    </aside>
  ...
  return (
    <label className="mt-4 block border-t border-[var(--app-border)] pt-4">
      ...
      <select
        ...
        className="min-h-10 w-full rounded-md border border-[var(--app-border)] bg-[var(--app-elevated)] px-3 text-xs font-black text-[var(--app-foreground)] outline-none transition hover:border-[var(--color-brand-primary)]/45"
      >
        {options.map((option) => (
          <option key={option} value={option} className="bg-[var(--app-foreground)] text-white">
            {option}
          </option>
        ))}
      </select>
...
function EventCard({ event, saved, onSave }: { event: EventItem; saved: boolean; onSave: (id: string) => void }) {
  return (
    <article className="group overflow-hidden rounded-md border border-[var(--app-border)] bg-[var(--app-elevated)] shadow-[0_10px_30px_rgba(15,23,42,0.12)] transition duration-200 hover:-translate-y-1 hover:border-[var(--color-brand-primary)]/40 hover:shadow-[0_20px_54px_rgba(15,23,42,0.18)]">
      ...
      <div className="p-3 sm:p-4">
        <Link href={event.href} className="line-clamp-1 text-sm font-black text-[var(--app-foreground)] transition hover:text-[var(--color-brand-primary)]">
          {event.title}
        </Link>
        ...
        <div className="mt-3 space-y-1.5 text-[11px] font-semibold text-[var(--app-muted)] sm:text-xs">
          ...
          <p className="flex items-center gap-1.5"><MapPin className="size-3.5 text-[var(--color-brand-primary)]" />{event.venue}</p>
          <p className="flex items-center gap-1.5"><Star className="size-3.5 fill-[var(--color-brand-accent)] text-[var(--color-brand-accent)]" />{event.rating} ({event.interested})</p>
        </div>
        <p className="mt-3 text-sm font-black text-[var(--app-foreground)]">{event.priceLabel}</p>
        <Link href={event.href} className="mt-3 inline-flex min-h-9 w-full items-center justify-center rounded-md bg-[var(--color-brand-primary)] px-4 text-xs font-black !text-white shadow-[0_14px_34px_rgba(236,27,114,0.24)] transition duration-200 hover:bg-[var(--color-brand-primary)]">
...
function WishlistButton({ event, saved, onSave, className }: { event: EventItem; saved: boolean; onSave: (id: string) => void; className?: string }) {
  return (
    <button
      ...
      className={`absolute grid size-8 place-items-center rounded-full bg-black/58 text-white shadow-md backdrop-blur transition duration-200 hover:bg-[var(--color-brand-primary)] ${saved ? "bg-[var(--color-brand-primary)] text-white" : ""} ${className ?? ""}`}
