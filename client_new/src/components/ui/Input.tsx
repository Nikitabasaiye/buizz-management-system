  ...
  ({ label, hint, error, leftIcon, rightIcon, loading, className, id, disabled, ...props }, ref) => {
    ...
    return (
      <label className="block">
        {label && (
          <span className="mb-2 block text-sm font-black text-[var(--color-text-primary)]">
            {label}
          </span>
        )}
        <span className="relative block">
          ...
          <input
            ...
            className={cn(
              "h-12 w-full rounded-[var(--radius-md)] border border-[var(--color-border-default)] bg-[var(--color-surface-elevated)] px-4 text-sm font-semibold text-[var(--color-text-primary)] outline-none transition placeholder:text-[var(--color-text-muted)] focus:border-[var(--color-brand-primary)] disabled:opacity-60",
