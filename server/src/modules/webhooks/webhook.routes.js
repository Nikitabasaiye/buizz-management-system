...
router.get('/whatsapp', (req, res) => {
  ...
  if (mode === 'subscribe' && token === WEBHOOK_VERIFY_TOKEN) {
    logger.info('Webhook verified successfully', { requestId });
...
router.post('/whatsapp', rateLimitMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (APP_SECRET) {
      if (!signature) {
        logger.warn('Webhook signature missing', { requestId });
        return res.status(403)
          .set('X-Request-ID', requestId)
          .json({ 
      ...
      const processingPromises = body.entry?.map(async (entry) => {
        ...
        for (const change of changes) {
          ...
          if (processedWebhooks.has(idempotencyKey)) {
            logger.info('Duplicate webhook skipped', { requestId, idempotencyKey });
            continue;
          }
          
          processedWebhooks.add(idempotencyKey);
          
          logger.info('Webhook field received', { 
            requestId, 
            field, 
            entryId: entry.id 
          });
          ...
          switch (field) {
            ...
            case 'business_capability_update':
              ...
              break;
            default:
              logger.info('Unhandled webhook field', { requestId, field });
          }
        }
      }) || [];
...
async function handleMessagesWebhook(value, requestId) {
  try {
    ...
    for (const message of messages) {
      ...
      switch (type) {
        case 'text':
          ...
          logger.info('Text message received', { requestId, from, textBody });
          // TODO: Implement chatbot logic here
          break;
        case 'interactive':
          const interactiveType = message.interactive?.type;
          logger.info('Interactive message received', { requestId, from, interactiveType });
          // TODO: Handle button/list responses
          break;
        case 'image':
          const imageId = message.image?.id;
          logger.info('Image message received', { requestId, from, imageId });
          // TODO: Handle image messages
          break;
        case 'document':
          const documentId = message.document?.id;
          logger.info('Document message received', { requestId, from, documentId });
          // TODO: Handle document messages
          break;
        case 'audio':
          const audioId = message.audio?.id;
          logger.info('Audio message received', { requestId, from, audioId });
          break;
        case 'video':
          const videoId = message.video?.id;
          logger.info('Video message received', { requestId, from, videoId });
          break;
        case 'location':
          const location = message.location;
          logger.info('Location message received', { requestId, from, location });
          break;
        case 'contacts':
          const contactsData = message.contacts;
          logger.info('Contacts message received', { requestId, from, contactsData });
          break;
        case 'reaction':
          const reaction = message.reaction;
          logger.info('Reaction message received', { requestId, from, reaction });
          break;
        case 'order':
          const order = message.order;
          logger.info('Order message received', { requestId, from, order });
          break;
        default:
          logger.info('Unknown message type', { requestId, from, type });
      }
    }
    ...
    logger.error('Error handling messages webhook', { requestId, error: error.message });
  }
}
...
async function handleTemplateStatusUpdate(value, requestId) {
  try {
    ...
    logger.error('Error handling template status update', { requestId, error: error.message });
  }
}
...
async function handleTemplateQualityUpdate(value, requestId) {
  try {
    logger.info('Template quality update received', {
      requestId,
      messageTemplateId: value.message_template_id,
    ...
    logger.error('Error handling template quality update', { requestId, error: error.message });
  }
}
...
async function handleAccountAlerts(value, requestId) {
  try {
    logger.info('Account alert received', {
      requestId,
    ...
    logger.error('Error handling account alert', { requestId, error: error.message });
  }
}
...
async function handlePhoneNumberNameUpdate(value, requestId) {
  try {
    logger.info('Phone number name update received', {
      requestId,
      phoneNumberId: value.phone_number_id,
    ...
    logger.error('Error handling phone number name update', { requestId, error: error.message });
  }
}
...
async function handlePhoneNumberQualityUpdate(value, requestId) {
  try {
    ...
    logger.error('Error handling phone number quality update', { requestId, error: error.message });
  }
}
...
async function handleBusinessCapabilityUpdate(value, requestId) {
  try {
    logger.info('Business capability update received', {
      requestId,
    ...
    logger.error('Error handling business capability update', { requestId, error: error.message });
  }
}
