    ...
    }),
    {
      ...
      migrate: (state) => {
        ...
        return {
          ...
          status: persisted?.user ? "authenticated" : "guest",
        };
      },
    }
  )
...
export function normalizeUser(user: PublicUser): PublicUser {
  ...
  const isEmailVerified = Boolean(email && ("isEmailVerified" in user ? user.isEmailVerified : true));
  const isPhoneVerified = Boolean(phone && user.isPhoneVerified);
