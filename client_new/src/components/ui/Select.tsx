  ...
  ({ label, hint, error, loading, className, id, disabled, children, ...props }, ref) => {
    ...
    return (
      <label className="block">
        {label && <span className="mb-2 block text-sm font-black text-[var(--color-text-primary)]">{label}</span>}
        <span className="relative block">
          <select
            ...
            className={cn(
              "h-12 w-full appearance-none rounded-[var(--radius-md)] border border-[var(--color-border-default)] bg-[var(--color-surface-elevated)] px-4 pr-11 text-sm font-semibold text-[var(--color-text-primary)] outline-none transition focus:border-[var(--color-brand-primary)] disabled:opacity-60",
